...
[~,sort_kb2] = sort(sqrt(((x(1,:) - x(6,:)).^2 + (y(1,:) - y(6,:)).^2 + (z(1,:) - z(6,:)).^2)));
...
%	% additional properties
	...
	if (length(extraprops)>0),
        for i_kb2 = 2:2:length(extraprops)
            if size(extraprops{i_kb2},1) == 1
                set(H,extraprops{i_kb2-1},extraprops{i_kb2})
            else
                for j_kb2 = 1:size(extraprops{i_kb2},1)
                    temp_kb2 = extraprops{i_kb2};
                    set(H(j_kb2),extraprops{i_kb2-1},temp_kb2(j_kb2,:))
                end
            end
        end
    end;
