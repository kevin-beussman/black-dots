function [px,py] = choose_dots(img,celldata,meta)
    ...
    function fig_buttonmotionfcn(~, ~, ~)
        if down
            ...
            curr_point_rot = (R*(curr_point - [N/2,M/2])')' + [N/2, M/2];
            init_point_rot = (R*(init_point - [N/2,M/2])')' + [N/2, M/2];
            ...
            temp_pc = (R2*([pxc_rot(:), pyc_rot(:)] - [N/2,M/2])')' + [N/2, M/2];
            pxc = temp_pc(:,1);
            pyc = temp_pc(:,2);
        ...
        elseif down_corner
            ...
            temp_pc = (R2*([pxc_rot_shift(:), pyc_rot_shift(:)] - [N/2,M/2])')' + [N/2, M/2];
            pxc = temp_pc(:,1);
            pyc = temp_pc(:,2);
            pxc = reshape(pxc,num_Yc,num_Xc);
            pyc = reshape(pyc,num_Yc,num_Xc);
