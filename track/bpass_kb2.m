function res = bpass_kb2(image_array,lnoise,lobject,threshold)
% 
% NAME:
%               bpass
% PURPOSE:
...
image_array = zeros(size(image_array_old) + lzero*2);
image_array((lzero+1):(end-lzero),(lzero+1):(end-lzero)) = image_array_old;

image_array((lzero+1):(end-lzero),1:lzero) = repmat(image_array_old(:,1),1,lzero);
image_array((lzero+1):(end-lzero),(end-lzero+1):end) = repmat(image_array_old(:,end),1,lzero);

image_array(1:lzero,(lzero+1):(end-lzero)) = repmat(image_array_old(1,:),lzero,1);
image_array((end-lzero+1):end,(lzero+1):(end-lzero)) = repmat(image_array_old(end,:),lzero,1);

image_array(1:lzero,1:lzero) = image_array_old(1,1);
image_array(1:lzero,(end-lzero+1):end) = image_array_old(1,end);
image_array((end-lzero+1):end,1:lzero) = image_array_old(end,1);
image_array((end-lzero+1):end,(end-lzero+1):end) = image_array_old(end,end);
%%% end of KB2 edit
...
filtered = filtered((lzero+1):(end-lzero),(lzero+1):(end-lzero));
%%% end of KB2 edit
