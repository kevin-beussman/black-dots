function [result] = bfopen_kb2(id, varargin)
...
% initialize logging
loci.common.DebugTools.enableLogging('WARN'); % set to 'INFO' to spill info
...
for s = 1:numSeries
%     fprintf('Reading series #%d', s); % KB2 commented
    r.setSeries(s - 1);
    ...
    if length(varargin) == 1
        images_to_get = varargin{1};
        varargin = cell(0);
    else
        images_to_get = 1:numImages;
    end
    imageList = cell(length(images_to_get), 2);
    colorMaps = cell(length(images_to_get));
    
%%% KB2 edit
% %     wbar = waitbar(0,'');
% %     set(findall(wbar,'type','text'),'Interpreter','none');
    pct = 0;
    nlen = length(num2str(numImages));
    fprintf(['[%-20s] %' num2str(nlen) '.0f/%-' num2str(nlen) '.0f\n'],repmat('|',1,round(pct*20)),0,numImages)
% %     [~, kb2_name, kb2_ext] = fileparts(id);
    for i = images_to_get
%         if mod(i, 72) == 1
%             fprintf('\n    ');
%         end
%         fprintf('.');
% %         waitbar(i/numImages, wbar, sprintf(['Reading file ' kb2_name kb2_ext '\n Image %i / %i'], i, numImages))
    pct = i/numImages;
    fprintf([repmat('\b',1,25+2*nlen) '[%-20s] %' num2str(nlen) '.0f/%-' num2str(nlen) '.0f\n'],repmat('|',1,round(pct*20)),i,numImages)
%%% end KB2 edit
